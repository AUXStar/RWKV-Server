...
class TaskManager:
    def __init__(
        ...
        self.worker = [None] * 2
        self.stream_compute = [torch.cuda.Stream()]*2
        self.stream_copy = [torch.cuda.Stream()]*2
        self.event_compute = torch.cuda.Event()
        self.clear_worker(0)
        self.clear_worker(1)
    ...
    def task_single(self, mask: torch.Tensor, pair: int):
        ...
        copy = self.stream_copy
        event = self.event_compute
        ...
        for slot_i in range(self.buffer_size):
            # 
            with torch.cuda.stream(compute[pair]):
                event.wait(compute[oth_pair])
                logits = self.pipeline.model.patch_forward_seq_batch(
                    last_tokens[: self.max_batch_pair_size].unsqueeze(-1), (state0, state1)
                )
                event.record(compute[pair])

            with torch.cuda.stream(copy[pair]):
                event.wait(compute[pair])
